...
const verifCandidature = async () => {
    if(authStore.isLoggedIn && userRole !== 'employeur'){
        try {
            ...
            const response = await axios.post('http://13.39.156.186:3001/api/v1/candidats/candidature', {
                    id_candidat: userId,
...
const candidate = async () => {
    try {
                const response = await axios.post('http://13.39.156.186:3001/api/v1/candidats/candidater', {
                    id_annonce: annonceId,
                    id_candidat: userId,
                    statut: 'en cours'
                });
...
const hydrateUser = async () => {
  try  {
        const response = await axios.get(`http://13.39.156.186:3001/api/v1/annonces/${annonceId}`);
