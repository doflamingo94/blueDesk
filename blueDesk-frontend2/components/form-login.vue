  ...
  if (newUser.value.typeUser === 'Candidat') {
    if (
      ...
      try {
        ...
        const response = await axios.post('https://13.39.156.186:3001/api/v1/candidats', {
    ...
    if (
      ...
      try {
        const response = await axios.post('https://13.39.156.186:3001/api/v1/employeurs', {
          nom: newUser.value.nom,
          mail: newUser.value.mail,
          phone: newUser.value.phone,
          pass: newUser.value.pass,
  ...
  if(userLog.value.typeUser === 'Employeur' && !userLog.value.identifiant || !userLog.value.pass){
    showError("Veuiller remplir tous les champs.")
        ...
        if (userLog.value.typeUser === 'Candidat') {
            const response = await axios.post('https://13.39.156.186:3001/api/v1/candidats/login', {
          ...
          const response = await axios.post('https://13.39.156.186:3001/api/v1/employeurs/login', {
